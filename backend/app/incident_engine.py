...
INCIDENT = {
    ...
    "severity": "SEV-2",
    "briefing": (
        "At 02:13 UTC, the production API began returning intermittent 504 errors. "
        "Error rate increased from 0.2% to 18%. CPU and memory are normal. "
        "Database connections are increasing. A deployment completed 17 minutes before "
        "the alert. You are the on-call DevOps engineer."
    )
}
...
Interpretation: the ALB itself appears healthy, while target response latency has increased.""",
...
A passing health check does not prove the application is healthy for real traffic.""",
...
A basic subnet/NACL/security filtering or routing failure is less likely.""",
...
Expected local routing exists between the application and RDS subnets.""",
...
def evidence_key(text: str):
    ...
    mappings = [
        ...
        (["rds", "database metrics", "db metrics", "database connections"], "rds"),
    ]
...
def interpret_request(text: str, state: IncidentState):
    ...
    if any(x in text.lower() for x in ["rollback", "roll back", "revert"]):
        state.actions.append(text)
        ...
        return (
            ...
            "Continue with root-cause analysis and explain why the rollback was justified.",
            "rollback",
        )

    if any(x in text.lower() for x in ["hypothesis", "i think", "i suspect", "likely cause"]):
        state.actions.append(text)
        state.hypotheses.append({"statement": text, "confidence": None})
        return (
            "Hypothesis recorded. Continue gathering evidence and tell me what "
            "signal would increase or decrease your confidence.",
            "hypothesis",
        )

    state.actions.append(text)
    return (
        "I can help with that, but make the investigation request concrete. "
        "For example: check ALB metrics, target health, application logs, "
        "VPC Flow Logs, route tables, security groups, RDS metrics, or deployment history.",
        "unknown",
    )
